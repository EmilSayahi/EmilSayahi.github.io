...
TheForm = {
	...
	formSubmission : function(event) {
		...
		if (typeof gifPlugin === 'undefined') {
			GifSound.gifFailed('No plugin could handle the given .GIF URL.');
			return;
		}
		...
		if (typeof soundPlugin === 'undefined') {
			GifSound.soundFailed('No plugin could handle the given sound URL.');
			return;
		}
...
GifSound = {
	...
	playIfSynced : function() {
		if (GifSound.s.gifReady && GifSound.s.soundReady) {
            ...
            iFrameResize({log:true,autoResize:true,sizeWidth:true,interval:-32,heightCalculationMethod:'grow'}, '#gifsound');
		}
	},
